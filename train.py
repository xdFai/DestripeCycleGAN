...
def main():
    ...
    for ep in range(ep0, opts.n_ep):
        ...
        # for it, (images_a, images_b, images_c) in enumerate(train_loader):
        for it, (images_a, images_b) in enumerate(train_loader):
            ...
            # operation Structure outside the discriminator
            ...
            print(
                'total_it: %d (ep %d, it %d), lr %08f, ganA %04f, WRGM %04f, recA %04f, recB %04f, identity %04f, percp %04f, total %04f' % (
                    total_it, ep, it, model.genA_opt.param_groups[0]['lr'], \
                    model.gan_loss_a, model.tvloss, \
                    model.l1_recon_A_loss, model.l1_recon_B_loss, \
                    model.identity_loss, model.perceptual_loss, model.G_loss))
        ...
        with torch.no_grad():
            for i, (input_val, target_val) in enumerate(loader_val, 0):
                ...
                out_val = torch.clamp(out_val, -1., 1.)
                out_val = (out_val + 1) / 2
